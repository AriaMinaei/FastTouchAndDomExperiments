...
class DommyContainer.Styles
	...
	getTransform: (fastId, el) ->
		return t if t = @dommy._get(fastId, 'style.transform')
		return @_initTransform(fastId, el)

	setTransform: (fastId, el, t) ->

	_initTransform: (fastId, el) ->
		prop = getComputedStyle(el).webkitTransform;
		t = new WebKitCSSMatrix(prop)
		@dommy._set(fastId, 'style.transform', t)
		@dommy._set(fastId, 'style.transform.original', t)
		t

	translate: (fastId, el, x, y) ->
		t = @getTransform(fastId, el).translate(x, y)
